...
Can reject null hypothesis that $\beta_1 = 0$ from simple linear regression using the p-value on the predictor. 

Alternatively, use the confidence intervals and see whether they contain 0. 
```{r}
confint(bw_model)
```

In this case, $\beta_1 = 0.47[0.41, 0.53]$ which provides evidence to reject the null hypothesis that $\beta_1 = 0$.

# QQ plot: checking assumptions of normality
```{r}
plot(bw_model, which = 2)
```
# Residual plot: homoskedacity and non-linearity check
```{r}
...
Note -- two ways to write this result in the same model
```{r}
int_model <- lm(Birth.Weight ~ Gestational.Days + factor(Maternal.Smoker) + Gestational.Days*factor(Maternal.Smoker), data = data)
summary(int_model)
```
These are equivalent! R lets you skip writing out the independent terms. Would recommend not using this to remind yourself of the full model you are fitting!
```{r}
int_model2 <- lm(Birth.Weight ~  Gestational.Days*factor(Maternal.Smoker), data = data)
summary(int_model2)
```
