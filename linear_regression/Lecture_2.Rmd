...
The "healthy" BMI category is our "reference" or baseline category -- so we can interpret the coeffecients $\beta$ as the expected change in baby birth weight as a result of being in each of the three other BMI categories. We could also change the reference category and this shifts the interpretation of the coefficients.
...
How do we interpret the intercept? $\beta_0$ is the estimated birth weight for a baby whose mother has a healthy BMI.
...
$$H0: \beta1 = 0, \beta2 = 0, \beta3 = 0$$ $$H1: \mathrm{at least one of} \beta1, \beta2, \beta3 \neq 0$$
...
Here, an F-value of 2.4 suggests that the model explains 2.4 times more variation per degree of freedom than the residual error, suggesting some relationship. p-value=0.065 some evidence against the null hypothesis of no association between BMI and baby birthweight.
...
$\beta_1$ = expected increase in a baby's birthweight for each gestational day for mothers of the same weight. $\beta_2$ = expected increase in a baby's birthweight for each additional pound of a mother's weight for babies of the same number of gestational days.
...
Redefine Model 4 using the centered variables $$ Y_i = \beta_0 + \beta_1(X_{1i} - \bar{X_1}) + \beta_2(X_{2i} - \bar{X_2}) + \epsilon_i $$
...
F-stat = 135 means the model explains 135 times more variation per degree of freedom than the residual error. P-value of \< 2.2e-16 indicates we have evidence that gestational days and maternal weight are associated with baby birth weight.
...
```{r}
ggplot(data) + geom_point(aes(y = Birth.Weight, x = Gestational.Days)) +
  theme_bw()
```
...
## F-test to test for quadratic terms
...
Model without the interaction term $$ Y_i = \beta_0 + \beta_1X_{1i} + \beta_2X_{2i} + \epsilon_i $$
...
Model with the interaction term: $$ Y_i = \beta_0 + \beta_1X_{1i} + \beta_2X_{2i} +  \beta_3X_{1i} \times X_{2i} + \epsilon_i $$
...
$\beta_3$ is the difference in slopes between the two groups defined by smoking status.
...
Can continue to test for the null hypotheses of the other variables e.g. $\beta_1$ and $\beta_2$ by creating a null model that removes those additional terms.

# Example 5: Continuous variable interaction terms, effect of weight and gestational days on birthweight

Same model, but here $X_1$ is mothers pregnancy weight and $X_2$ is gestational days.
...
$\beta_3$ is the change in slope (maternal weight on baby birthweight) per unit increase in pregnancy weight, or the change in slope (pregnancy weight on baby birthweight) per unit increase in length of pregnancy.
