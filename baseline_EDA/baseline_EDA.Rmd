...
```
As can be seen, the first figure represents only a very small proportion of the
...
ggplot(confirm_long |> dplyr::left_join(mean_delay, by = "delay")) +
  geom_line(aes(x = delay, y = new_confirm/max_confirm, 
                ...
                group = reference_date),
              alpha = 0.1) + 
  geom_line(aes(x = delay, y = exp_prop_report), color = "black", size = 1.2)+
...
for(i in 1:n_draws){
  ...
  # by drawing from a negative binomial with dispersion from the size estimate
  # and the expectation from dividing by the inverse of the cumulative 
  # number of expected reports. We'll pad for 0s in the confirm for now, will
  # come up with something more robust and estimated later.
  nc_i$exp_max_confirm <- rnbinom(n = nrow(nc_i), 
                                  size = size_estimate,
                                  mu = (nc_i$confirm+abs(rnorm(nrow(nc_i), 1e-5,3)))/nc_i$exp_cum_report)
