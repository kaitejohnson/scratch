...
for (i in 1:nrow(df_w_expectations)){
  df_w_sims$sim_confirm[i] <-  rnbinom(n = 1,
                               mu = df_w_expectations$max_confirm[i]*df_w_expectations$exp_prop_report[i], 
                               size = size_estimate)
}
...
# Find the dispersion in the observation model
fit_nb <- function(x, mu){
  nllik <- function(size){-sum(dnbinom(x = x, mu = mu, size = size, log = TRUE), na.rm = TRUE)}
  opt <- optimize(nllik, c(0.1, 1000))
  opt$minimum
